...
def init_dir(autofz_dir: pathlib.Path) -> None:
    '''
...
class QSYMController(Controller):
    ...
    def init(self):
        ...
        self.db.connect()
        self.db.create_tables([QSYMModel, ControllerModel])
    ...
    def start(self):
        ...
        args = afl_master.gen_run_args()
        args = " ".join(args)
        # 创建autofz目录下的子目录 queue/crashes/hang, 并创建fuzzer_stats文件伪装成一个afl实例
        autofz_dir = os.path.join(self.output, 'autofz')
        init_dir(autofz_dir)
        fuzzer_stats_path = os.path.join(autofz_dir, 'fuzzer_stats')
        pathlib.Path(fuzzer_stats_path).touch(mode=0o666, exist_ok=True)
        # print(args)
        command_line = "command_line :" + args
        with open(fuzzer_stats_path, 'w') as f:
            # 写入args内容
        ...
        qsym = QSYM(**self.kwargs, afl_name='autofz')
        qsym.start()
        QSYMModel.create(**self.kwargs, afl_name='autofz', pid=qsym.pid)
