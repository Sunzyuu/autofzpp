...
# TODO
class AFLSMART(AFLBase):

    @property
    def afl_command(self):
        global FUZZER_CONFIG
        return FUZZER_CONFIG['aflsmart']['command']
...
# todo
class AFLSMARTController(AFLBasedController):
    def __init__(self, seed, output, group, program, argument, thread,
                 cgroup_path):
        super().__init__(AFLSMART, 'aflsmart', seed, output, group, program, argument,
                         cgroup_path)
        self.db = peewee.SqliteDatabase(
            os.path.join(Config.DATABASE_DIR, 'autofz-aflsmart.db'))
